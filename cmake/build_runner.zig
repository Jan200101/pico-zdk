...
pub fn main() !void {
    ...
    while (nextArg(args, &arg_idx)) |arg| {
        if (mem.startsWith(u8, arg, "-Z")) {
            ...
            if (mem.eql(u8, arg, "--verbose")) {
                ...
                if (step_name != null) {
                    std.debug.print("step already specified", .{});
                    process.exit(1);
                }
                const next_arg = nextArgOrFatal(args, &arg_idx);
                step_name = next_arg;
    ...
    var run: Run = .{
        ...
        .claimed_rss = 0,
        .summary = summary orelse .failures,
        .ttyconf = ttyconf,
        .stderr = stderr,
...
const TargetInfo = struct {
    ...
    pub fn jsonStringify(self: *const @This(), jws: anytype) !void {
        ...
        try jws.objectField("linkage");
        try jws.write(self.linkage orelse .static);

        try jws.objectField("emitted_path");
        try jws.write(self.emitted_path);

        try jws.endObject();
    }
};

fn serializeSteps(b: *std.Build) !void {
    const arena = b.graph.arena;

    var steps: std.ArrayList(TargetInfo) = .empty;
    ...
    for (b.top_level_steps.values()) |tls|
        try serializeStepsNested(b, &steps, &seen, &tls.step);

    var w = initStdoutWriter();
    defer w.flush() catch {};

    try std.json.Stringify.value(steps.items, .{}, w);
}

fn serializeStepsNested(
    b: *std.Build,
...
fn appendUnseenStep(
    ...
    try steps.append(b.graph.arena, .{
        .name = compile_step.name,
        .kind = compile_step.kind,
        .linkage = compile_step.linkage,
    });
